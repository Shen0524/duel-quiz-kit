...
(function () {
  ...
  function lockAfterAnswer(q, choiceBool, room) {
    ...
    const verdictWord = q.answer ? "這個說法是「正確的」。" : "這是「謠言／錯誤說法」。";
    fb.innerHTML = head +
    ...
    S.questions.forEach((q, i) => {
      ...
      html += `<div class="review-item">
        ...
        <div class="meta" style="margin-top:6px;color:var(--muted);font-size:12px;line-height:1.6">
          正解：${q.answer ? "正確" : "謠言"}　|　${esc(q.explain)}</div>
      </div>`;
    });
    ...
    ref("stats").get().then(snap => {
      ...
      $("statsList").innerHTML = answered.map(r => {
        ...
        return `<div class="stat-row">
          ...
          <div class="meta">作答 ${r.att} 次，答錯 ${r.wrong} 次　·　正解：${r.q.answer ? "正確" : "謠言"}　·　${esc(r.q.category)}</div>
        </div>`;
      }).join("");
    }).catch(err => {
      console.error(err);
